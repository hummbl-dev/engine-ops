...
const ENGINE_URL =
  vscode.workspace.getConfiguration('hummbl').get<string>('engineUrl') || 'http://localhost:8080';
...
Do not include any explanation or markdown formatting, only the JSON object.`,
        ...
        if (response.status !== 200) {
          throw new Error(
            `Engine responded with status ${response.status}: ${response.statusText}`,
          );
        }
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            return new vscode.ChatResponse(
              `Error: Engine returned status ${response.status}. Make sure the engine is running at ${ENGINE_URL}. Original error: ${error?.message || error}`,
            );
          }
        } catch (fallbackError: any) {
          console.error('[HUMMBL Extension] Fallback error:', fallbackError);
          return new vscode.ChatResponse(
            `Error consulting council: ${error?.message || error}. Make sure the engine is running at ${ENGINE_URL}. Connection error: ${fallbackError?.message || fallbackError}`,
          );
        }
      }
    },
