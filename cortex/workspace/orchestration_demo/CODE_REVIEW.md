...
- **`debug=True` in Production:** The most critical security flaw. Running `app.run(debug=True)` exposes the Werkzeug debugger, which can execute arbitrary Python code on the server, making it a severe remote code execution vulnerability. This _must_ be disabled in production.
