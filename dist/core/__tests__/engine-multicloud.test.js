...
(0, globals_1.describe)('OptimizationEngine - Multi-Cloud Integration', () => {
    ...
    (0, globals_1.describe)('Multi-Cloud Initialization', () => {
        ...
        (0, globals_1.it)('should initialize with all providers when multi-cloud is enabled', async () => {
            engine = new engine_1.OptimizationEngine({
    ...
    (0, globals_1.describe)('Multi-Cloud Workload Scheduling', () => {
        ...
        (0, globals_1.it)('should throw error when multi-cloud is not enabled', async () => {
            ...
            await (0, globals_1.expect)(engineNoMultiCloud.scheduleMultiCloudWorkloads(workloads)).rejects.toThrow('Multi-cloud is not enabled');
        });
        (0, globals_1.it)('should return partial success when some workloads cannot be scheduled', async () => {
            const workloads = [
                {
                    id: 'schedulable',
                    resources: { cpu: 2000, memory: 4000 },
            ...
            const result = await engine.scheduleMultiCloudWorkloads(workloads);
            (0, globals_1.expect)(result.success).toBe(true);
            (0, globals_1.expect)(result.result.totalScheduled).toBeLessThan(result.result.totalRequested);
        });
    });
