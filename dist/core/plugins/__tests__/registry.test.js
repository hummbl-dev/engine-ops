...
(0, globals_1.describe)('PluginRegistry', () => {
  ...
  (0, globals_1.it)('should emit events on plugin lifecycle', async () => {
    const plugin = new TestPlugin();
    const events = [];
    registry.on(interfaces_js_1.PluginEvent.REGISTERED, () =>
      events.push(interfaces_js_1.PluginEvent.REGISTERED),
    );
    registry.on(interfaces_js_1.PluginEvent.UNREGISTERED, () =>
      events.push(interfaces_js_1.PluginEvent.UNREGISTERED),
    );
    await registry.register(plugin);
    await registry.unregister('test-plugin');
