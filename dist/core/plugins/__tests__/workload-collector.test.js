...
(0, globals_1.describe)('WorkloadCollector', () => {
    ...
    (0, globals_1.it)('should get recent data', () => {
        for (let i = 0; i < 5; i++) {
            collector.record({ id: `test-${i}`, type: 'resource', data: {} }, { requestId: `test-${i}`, success: true, metrics: { durationMs: 100, score: 1 } });
        }
    ...
    (0, globals_1.it)('should maintain max size', () => {
        ...
        for (let i = 0; i < 10; i++) {
            smallCollector.record({ id: `test-${i}`, type: 'resource', data: {} }, { requestId: `test-${i}`, success: true, metrics: { durationMs: 100, score: 1 } });
        }
        const data = smallCollector.getAllData();
        (0, globals_1.expect)(data).toHaveLength(5);
    });
    (0, globals_1.it)('should clear all data', () => {
        collector.record({ id: 'test-1', type: 'resource', data: {} }, { requestId: 'test-1', success: true, metrics: { durationMs: 100, score: 1 } });
        collector.clear();
