...
async function fetchMetrics() {
  try {
    ...
    document.getElementById('avg-latency').textContent = metrics.avgDuration
...
async function fetchHealth() {
  try {
    ...
    updateHealthCard(
      'api-health',
      'down',
      'Cannot connect to ' + API_BASE + '. Start backend with: npm start',
    );
  ...
  container.innerHTML = anomalies
    .map((anomaly) => {
      ...
      if (anomaly.remediation) {
        remediationHtml =
          '<p class="anomaly-description"><strong>💡 Remediation:</strong> ' +
          anomaly.remediation +
          '</p>';
      }
      let playbookHtml = '';
      if (playbookLink) {
        playbookHtml =
          '<a href="' +
          playbookLink +
          '" target="_blank" style="color: #818cf8; text-decoration: none; font-size: 0.875rem;">📖 View Playbook →</a>';
      }
      return (
        ...
        '</strong>' +
        '<span class="anomaly-severity severity-' +
        anomaly.severity +
        '">' +
        ...
        (anomaly.hypothesis || 'AI analysis in progress...') +
        '</p>' +
        remediationHtml +
        '<div style="margin-top: 0.75rem; display: flex; gap: 0.5rem;">' +
        playbookHtml +
        '<a href="#" onclick="acknowledgeAnomaly(\'' +
        (anomaly.id || Date.now()) +
        '\'); return false;" style="color: #10b981; text-decoration: none; font-size: 0.875rem;">✓ Acknowledge</a>' +
        '</div>' +
        '</div>'
      );
    })
...
function updateHealthCard(cardId, status, details = '') {
  ...
  if (status === 'healthy') {
    ...
    diagnostic.innerHTML =
      '⚠️ ' + (details || 'Service unavailable. Check logs or restart service.');
