...
export class MultiCloudResourceManager implements IResourceManager {
  ...
  async getUtilization(): Promise<
    Record<CloudProvider, { total: ResourceCapacity; used: ResourceCapacity }>
  > {
    const utilization: Partial<
      Record<CloudProvider, { total: ResourceCapacity; used: ResourceCapacity }>
    > = {};
    ...
    return utilization as Record<
      CloudProvider,
      { total: ResourceCapacity; used: ResourceCapacity }
    >;
  }
  ...
  private scoreNodes(
    nodes: CloudNode[],
    workload: Workload,
  ): Array<{ node: CloudNode; score: number; reason: string }> {
