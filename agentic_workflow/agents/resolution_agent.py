...
class ResolutionAgent(Agent):
    ...
    def process(self, context: AgentContext) -> AgentContext:
        ...
        self.telemetry.record_metric(
            "issues_resolved", resolution_results.get("resolved_count", 0), agent_id=self.agent_id
        )

        self.telemetry.record_metric(
            "resolution_failures", resolution_results.get("failed_count", 0), agent_id=self.agent_id
        )
    ...
    def _resolve_issues(
        self, issues: List[Dict[str, Any]], context: AgentContext
    ) -> Dict[str, Any]:
        """
    ...
    def _apply_resolution_strategy(
        self, issue: Dict[str, Any], strategy: Dict[str, Any], context: AgentContext
    ) -> tuple[bool, str]:
        """
        ...
        for issue in critical_issues:
            try:
                ...
                debate_result = orchestrator.orchestrate_debate(
                    issue, max_rounds=3, convergence_threshold=0.7
                )
                ...
                resolution_item = {
                    **issue,
                    ...
                    "resolution_details": decision.get(
                        "resolution_details", decision.get("rationale", "")
                    ),
                    "debate_metadata": {
                ...
                self.telemetry.error(
                    f"Debate failed for {issue.get('name')}: {e}", agent_id=self.agent_id
                )
                failed.append(
                    {**issue, "resolution_status": "failed", "reason": f"Debate error: {str(e)}"}
                )
    ...
    def _standard_resolution(self, issues: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.telemetry.error(
                f"Failed to parse resolution response: {e}", agent_id=self.agent_id
            )
            return {
                "resolved": [],
                "failed": issues,
