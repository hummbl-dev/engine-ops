...
class DetectionAgent(Agent):
    ...
    def process(self, context: AgentContext) -> AgentContext:
        ...
        # Record metrics
        self.telemetry.record_metric("detections_found", len(detections), agent_id=self.agent_id)

        # Update state
        context.update_state("detection_complete", {"detections": len(detections)})
    ...
    def _detect_issues(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if err_rate is not None:
            rule = next(
                (r for r in self.detection_rules if r["rule_id"] == "high_error_rate"), None
            )
        ...
        rule_res = next(
            (r for r in self.detection_rules if r["rule_id"] == "resource_exhaustion"), None
        )
        if rule_res:
        ...
        rule_anom = next(
            (r for r in self.detection_rules if r["rule_id"] == "anomalous_pattern"), None
        )
        if rule_anom:
